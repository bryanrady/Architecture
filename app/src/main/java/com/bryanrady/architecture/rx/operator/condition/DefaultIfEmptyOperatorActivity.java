...
public class DefaultIfEmptyOperatorActivity extends BaseActivity {
    ...
    public void initView(View view) {
        ...
        tv_operator_name.setText("DefaultIfEmpty");
        tv_operator_effect.setText("在不发送一个有效事件(next事件)、仅发送了complete事件的前提下，发送一个默认值");
    }

    private void initToolbar() {
        llBack.setVisibility(View.VISIBLE);
        tvToolbarTitle.setText("条件操作符---DefaultIfEmpty");
        mToolbar.setTitle("");
   ...
   // distinctUntilChanged 过滤掉连续重复的数据
    ...
    private void testOperator(){
        tv_send.append("Observable send : 被观察者发送一个empty()事件" + "\n");
        Observable.empty()
                .defaultIfEmpty("默认值为0")
